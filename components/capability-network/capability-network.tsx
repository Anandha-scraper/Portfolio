...
// CapabilityNetwork is itself lazy-loaded (it's below the fold, see app/page.tsx).
...
export function CapabilityNetwork() {
  ...
  const select = useCallback(
    (i: number) => {
      ...
      if (reduce) {
        // No sailing under reduced motion — dock at once.
    ...
    <section id="capabilities" className="ops ops-scanlines relative w-full overflow-hidden scroll-mt-24">
      ...
      <div className="relative z-5 w-full px-3 pt-2 pb-12 sm:px-5 sm:pb-16">
        ...
        {/* Voyage stage — full width. The stage carries the dark-ocean backdrop
            (capabg) behind the grid / islands / ship, plus its nine-slice
            dungeon-wall border. */}
        <div
          ref={stageRef}
