...
/**
 * TargetCursor (ported from React Bits, JS+CSS → TSX). A spinning bracket cursor
 * whose four corners lock onto any element matching `targetSelector` while the
 * pointer is over it. Ported faithfully; only typing + a mobile short-circuit
 * differ from the original. In this project it is mounted, scoped, inside the
 * Capability Dungeon panel with `hideDefaultCursor={false}` so it coexists with
 * the global custom key-cursor — only the tech-icon buttons carry `.cursor-target`.
 */
